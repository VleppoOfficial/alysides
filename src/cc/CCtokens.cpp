...
std::string CreateToken(int64_t txfee, int64_t tokensupply, std::string name, std::string description, double ownerPerc, std::string tokenType, uint256 referenceTokenId, int64_t expiryTimeSec, vscript_t nonfungibleData)
{
    ...
    // work on this for task 2 and 3 (task 4 will be in a if statement for itself prob below this statement)
    if (tokenType == "s" && (refExpiryTimeSec = 0)) 
	{
        CCerror = "You can't create a Sub-Licens if the Master-Licens is expired";
        LOGSTREAM((char*)"cctokens", CCLOG_INFO, stream << "CreateToken() " << CCerror << std::endl);
        return std::string("");
    }
        else if (tokenType == "s" && (expiryTimeSec > CCduration(numblocks, referenceTokenId)))
    {
            CCerror = "Sub-Licens is not allowed to have a longer timespand then Master-Licens";
            LOGSTREAM((char*)"cctokens", CCLOG_INFO, stream << "CreateToken() " << CCerror << std::endl);
            return std::string("");   
	}
