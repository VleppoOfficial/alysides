...
bool TokensValidate(struct CCcontract_info* cp, Eval* eval, const CTransaction& tx, uint32_t nIn)
{
	...
	*/
    ...
    else if (funcid != 'c') //in case these tokens have been transferred before
	{
		...
		if (!eval->GetConfirmed(tokenid, tokenBaseTx, hashBlock, false) || DecodeTokenCreateOpRet(tokenBaseTx.vout[numvouts - 1].scriptPubKey, creatorPubkey, dummyName, dummyDescription, ownerPerc, tokenType, referenceTokenId, expiryTimeSec, oprets) != 'c')
		{
